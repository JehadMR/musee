  ...
  return (
    ...
    {/*  {loading === false? 
      ...
      {tracks?.items?.slice(0, 6).map((item, i) => (
        ...
        <img  className='border border-none rounded-3xl group-hover:shadow-2xl' key={item.album.images[0].url} src={item.album?.images[0]?.url} width="100%" />
        <div className="flex flex-col justify-start items-center mt-3" key={item.artists[0]}>
        <h6 className="text-sm font-medium text-slate-800 " key={item.name}>{item.name}</h6>
        <p className=" text-xs font-medium text-slate-500" key={item.artists[0].name}>{item.artists[0].name}</p>
        </div>
        </div>
